...
const BODY_TEXT_CLASS = "text-stone-500 font-light text-base leading-relaxed";
...
export const MasteryOf4Cs: React.FC = () => {
  ...
  return (
    <section className="bg-white border-y border-stone-100 overflow-hidden py-32">
      <div className="max-w-[1600px] mx-auto px-8 mb-24">
        <div className="flex flex-col items-center text-center">
          <div className="space-y-12 max-w-4xl">
            <div className="space-y-4">
              <h2 className="text-5xl md:text-7xl font-serif text-stone-900 leading-tight tracking-tight">Mastery of the 4Cs</h2>
            </div>
            
            <div className="space-y-4 max-w-2xl mx-auto border-t border-stone-100 pt-12">
              <p className="text-stone-900 font-serif text-xl md:text-2xl leading-relaxed">
      ...
      <div className="max-w-[1400px] mx-auto px-8">
        <div className="flex justify-center border-b border-stone-100 overflow-x-auto no-scrollbar">
          {tabs.map((tab) => (
            <button
              key={tab.label}
              onClick={() => setActiveTab(tab.label)}
              className={`group px-8 md:px-16 py-8 transition-all relative flex flex-col items-center space-y-2 ${
                activeTab === tab.label ? 'text-stone-900' : 'text-stone-300 hover:text-stone-500'
              }`}
            >
              <span className="text-[11px] uppercase tracking-[0.5em] font-bold">{tab.label}</span>
              {activeTab === tab.label && (
                <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-stone-900 z-10"></div>
              )}
              <div className="absolute bottom-0 left-0 right-0 h-0 group-hover:h-[1px] bg-stone-200 transition-all"></div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-12 mt-12">
        <div className="bg-[#FBFBFA] border border-stone-100 min-h-[800px] relative shadow-sm overflow-hidden">
          <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{backgroundImage: 'radial-gradient(circle, #000 0.5px, transparent 0.5px)', backgroundSize: '30px 30px'}}></div>
...
const StandardSeal = ({ text }: { text: string }) => (
  <div className="pt-12 border-t border-stone-100 w-full max-w-sm mx-auto">
    <div className="flex flex-col space-y-3 items-center text-center">
      <span className="text-[9px] uppercase tracking-[1em] font-bold text-stone-900">ORÉA Standard</span>
      <p className="text-sm text-stone-500 font-light tracking-wide">{text}</p>
    </div>
  </div>
);
    ...
    <div className="p-12 lg:p-24 flex flex-col items-center gap-16">
      <div className="w-full max-w-3xl space-y-12 text-center">
        <div className="space-y-6">
          <h3 className="text-4xl md:text-6xl font-serif text-stone-900 leading-tight">The Architecture of Light</h3>
      ...
      <div className="w-full max-w-6xl space-y-2">
        <div className="w-full border-b-[2px] border-stone-900 flex">
          {['EXCELLENT', 'VERY GOOD', 'GOOD', 'FAIR', 'POOR'].map((label, idx) => (
            <div key={idx} className="flex-1 border-x border-stone-100 py-6 text-center">
              <span className="text-[9px] font-bold tracking-[0.2em] text-stone-900">{label}</span>
            </div>
          ))}
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-stone-100 mt-12">
          {[
            { 
              label: 'Ideal Cut', 
              type: 'ideal', 
            ...
            <div key={item.label} className="flex flex-col items-center space-y-8 bg-white p-12 group transition-colors duration-700">
                 ...
                 <svg viewBox="0 0 160 160" className="w-full h-full fill-none">
                  <defs>
                    <marker id="arrowhead-cut-new" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto">
                      <path d="M0,0 L6,3 L0,6 Z" fill="#C5A57D" />
                    </marker>
                  </defs>
                  ...
                  <g stroke="#C5A57D" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="flex flex-col items-center space-y-2 text-center">
                <h4 className="text-[11px] font-bold text-stone-900 tracking-[0.1em] uppercase">{item.label}</h4>
                <p className="text-[10px] text-stone-400 font-light tracking-wide">{item.note}</p>
              </div>
            </div>
          ))}
        </div>
...
const ColorDiamondSVG = ({ tint }: { tint: string }) => (
  <svg viewBox="0 0 100 70" className="w-full h-full stroke-stone-900 stroke-[0.8] fill-none">
  ...
  return (
    <div className="animate-reveal flex flex-col h-full">
      <div className="p-12 lg:p-24 flex flex-col items-center gap-20">
        <div className="w-full max-w-3xl space-y-12 text-center">
          <div className="space-y-6">
            <h3 className="text-4xl md:text-6xl font-serif text-stone-900 leading-tight">The Absence of Tint</h3>
            <div className="space-y-8 max-w-lg mx-auto">
              <p className={BODY_TEXT_CLASS}>
        ...
        <div className="w-full max-w-4xl bg-white p-12 border border-stone-100 shadow-sm relative overflow-hidden">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            {colorGroups.map((group, i) => (
              <div key={i} className="flex flex-col items-center space-y-8 group">
                <span className="text-[12px] font-bold tracking-[0.2em] text-stone-900 uppercase">{group.grade}</span>
                ...
                <div className="text-center">
                  <p className="text-[8px] uppercase tracking-[0.4em] font-bold text-stone-400">{group.name}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-stone-100 via-stone-300 to-stone-100"></div>
        </div>
      </div>
    </div>
  );
...
const ClarityChapter = () => (
  <div className="animate-reveal flex flex-col h-full">
    <div className="p-12 lg:p-24 flex flex-col items-center gap-20">
      <div className="w-full max-w-3xl space-y-12 text-center">
        <div className="space-y-6">
          <h3 className="text-4xl md:text-6xl font-serif text-stone-900 leading-tight">A Flawless Lens</h3>
          <div className="space-y-8 max-w-lg mx-auto">
            <p className={BODY_TEXT_CLASS}>
      ...
      <div className="w-full max-w-5xl space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-5 border border-stone-100 bg-white">
          {[
            ...
            <div key={i} className="flex flex-col border-r last:border-r-0 border-stone-100 group">
              <div className="relative aspect-square flex items-center justify-center p-6 bg-white border-b border-stone-50 overflow-hidden">
                <svg viewBox="0 0 100 100" className="w-full h-full stroke-stone-900 fill-none stroke-[0.8] transition-all duration-700 group-hover:scale-105">
                  <circle cx="50" cy="50" r="48" className="stroke-stone-900" strokeWidth="0.8" />
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="relative w-16 h-16">
                    {Array.from({ length: g.dots }).map((_, idx) => (
                      <div 
                        key={idx} 
                        className="absolute bg-stone-950 rounded-full" 
                        style={{ 
              ...
              <div className="py-6 flex flex-col items-center justify-center bg-white text-center px-2">
                <span className="text-[12px] font-bold tracking-widest text-stone-950 uppercase mb-2">{g.grade}</span>
                <p className="text-[7px] text-stone-400 font-bold uppercase tracking-[0.2em] leading-tight">{g.line1}</p>
                {g.line2 && <p className="text-[7px] text-stone-400 font-bold uppercase tracking-[0.2em] leading-tight">{g.line2}</p>}
              </div>
            </div>
          ))}
        </div>
...
const CaratChapter = () => (
  <div className="animate-reveal flex flex-col h-full">
    <div className="p-12 lg:p-24 flex flex-col items-center gap-20">
      <div className="w-full max-w-3xl space-y-12 text-center">
        <div className="space-y-6">
          <h3 className="text-4xl md:text-6xl font-serif text-stone-900 leading-tight">The Measure of Presence</h3>
          <div className="space-y-8 max-w-lg mx-auto">
            <p className={BODY_TEXT_CLASS}>
      ...
      <div className="w-full max-w-4xl bg-white border border-stone-100 p-12 lg:p-16 shadow-sm relative overflow-hidden">
          ...
          {[
            ...
            <div key={i} className="flex flex-col items-center group flex-1">
              <div className="flex flex-col items-center mb-10 h-32 flex justify-end">
                ...
                >
                  <svg viewBox="0 0 100 100" className="w-full h-full fill-none stroke-stone-900 stroke-[0.8]">
              ...
              <div className="text-center pt-6 border-t border-stone-100 w-full">
                <span className="text-[12px] font-bold tracking-[0.4em] text-stone-950 uppercase">{c.carat}</span>
              </div>
            </div>
          ))}
        </div>
